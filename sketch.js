...
let gameState = 'playing'; // 新增：遊戲主狀態 'playing', 'completed'
...
function draw() {
  ...
  if (distance4 < proximityThreshold4) {
    ...
    if (char4State === 'asking') {
      ...
      drawTextBox(textBoxText, char4ScreenX, char4Y, textYOffset4);
      // 顯示回饋 3 秒後，重置狀態
  ...
  if (char5State === 'asking' || char5State === 'giving_hint' || char5State === 'declined_hint') {
  ...
  if (distance2 < proximityThreshold2) {
    ...
    if (textBoxText) {
      ...
      drawTextBox(textBoxText, char2X + bgX - 50, char2Y, textYOffset);
